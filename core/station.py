...
from core.utils import get_distance, nkg, poisson_gauss_gen, gen_rndm_time, light_speed
...
class Station:
    ...
    def __init__(self, number, coordinates):
        ...
        self.time = None  # Время станции [нс]
    ...
    def set_state(self, evt_station):
        """Устанавливаем состояние станции в соответствии с
        прчитанным событием"""
        self.amplitude = evt_station['ampl']
        self.time = evt_station['time']

        if self.amplitude > st_threshold and self.time is not None:
            self.respond = True
        else:
            self.respond = False
            self.amplitude = 0.0
    ...
    def model_ampl(self, eas):
        ...
        for det in self.detectors:
            dist = get_distance(det['coord'], eas.vector, eas.x0, eas.y0)
            temp = det_area * abs(eas.vector[2]) * nkg(dist, eas.power, eas.age)
        ...
        self.add_det['ampl'] = gen_amplitudes(self.add_det['particles']) / 100
    ...
    def model_times(self, eas):
        ...
        for det in self.detectors:
            ...
            if det['respond']:
                det['time'] = (sum(eas.vector * det['coord']) + eas.D) / light_speed
                det['time'] += gen_rndm_time()
        ...
        if self.add_det['respond']:
            self.add_det['time'] = (sum(eas.vector * self.add_det['coord']) + eas.D) / light_speed
            self.add_det['time'] += gen_rndm_time()
        else:
            self.add_det['time'] = None
